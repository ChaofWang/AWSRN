    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_all
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile(methods{idx_method},  dataset{idx_set} , ['*_x', num2str(scale), '_HR', ext{idx_ext}])));
                sr_filepaths = cat(1, sr_filepaths, dir(fullfile(methods{idx_method},  dataset{idx_set} , ['*_x', num2str(scale), '_SR', ext{idx_ext}])));
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                name_SR = sr_filepaths(idx_im).name;
                 ...
                 im_HR = imread(fullfile(methods{idx_method}, dataset{idx_set},  name_HR));
                 im_SR = imread(fullfile(methods{idx_method},  dataset{idx_set},  name_SR));
